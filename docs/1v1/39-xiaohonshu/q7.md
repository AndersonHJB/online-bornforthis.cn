...
1. State for each question, whether the statements are True or False, and the reason why.

a. Linear searching does not need to usean ordered list, but abinary search does.

> True
>
> 线性搜索不需要使用有序的列表，因为它从列表的第一个元素开始，逐一检查每个元素，直到找到所需的元素或检查完整个列表。而二分搜索确实需要一个有序的列表，因为它的搜索策略是基于每次将搜索范围缩小一半的原理。如果列表不是有序的，那么二分搜索就无法正常工作。

b. The maximum number of values examined in a binary search of 40.000 items is the same asthat for 60,000 items.

> 二分搜索的核心思想是每次查找都将当前的搜索范围缩小一半。所以要查找的最大次数是以2为基数的对数值。
...
2. Suppose you have a list of names: 

**Andrew, Bob, Cathy, Debra, Kasim, Maisie, Noah, Paul, Sue, Tim, Verity** 

In a binary search of the list to find the name Kasim, which names would be examined?	[4] 
...
def binary_search(names, target):
    ...
    while left <= right:
        mid = (left + right) // 2
        mid_name = names[mid]
        examined_names.append(mid_name)

        if mid_name == target:
            return examined_names
...
names = ["Andrew", "Bob", "Cathy", "Debra", "Kasim", "Maisie", "Noah", "Paul", "Sue", "Tim", "Verity"]
target_name = "Kasim"
examined = binary_search(names, target_name)
print(examined)
```
...
For the 2nd and 3rd test cases, i.e. 2.in and 3.in,
