---
title: 09-多路加速，了解多线程基本原理
date: 2022-11-22 10:52:07
author: AI悦创
isOriginal: true
...
![](https://raw.githubusercontent.com/AndersonHJB/blog-images/refs/heads/main/docs-images/sha256/3f/3f14c27125c31ee0d9289aea6ff4b7a35da33281acfd3d59d0e950c5021496f8.png)
...
英文叫作 `parallel`。它是指同一时刻，有**多条指令在多个处理器上同时执行**，并行必须要依赖于多个处理器。不论是从宏观上还是微观上，多个线程都是在同一时刻一起执行的。
...
![](https://raw.githubusercontent.com/AndersonHJB/blog-images/refs/heads/main/docs-images/sha256/4c/4c499df9b6e2bc7731b1f363bfe2c546c59810e7bd0abb996f33d48fc0e3a94f.png)
...
![](https://raw.githubusercontent.com/AndersonHJB/blog-images/refs/heads/main/docs-images/sha256/41/4121b44b731821df28a1d570ce66a0121fcdfae70ff28aa7f33d13cd04cd11e9.png)
...
### 过期代码更新「thread.daemon = True」2022-11-23 13:48:02
...
print("start")  # 主线程开始
thread = threading.Thread(target=start, name="Cava", args=(1,))
# thread.setDaemon(True)
thread.daemon = True
thread.start()
print("stop")  # 主线程
...
**比方**说我们现在有两个线程，一个是求加一千万次，另一个是减一千万次。

按原本得计划来说，一个加一千万一个减一千万结果应该还是零。可是最终得结果并不是等于零，我们多运行几次会发现几次得出来得结果并不相同。

::: details 此部分代码待考究，选择性阅读
...
print("start")
add_thread = threading.Thread(target=addNumber, args=(2,))
down_thread = threading.Thread(target=downNumber, args=(2,))
add_thread.start()
down_thread.start()
print("外面：number:>>>", number)
print("stop")
```
...
![](https://raw.githubusercontent.com/AndersonHJB/blog-images/refs/heads/main/docs-images/sha256/79/79f6a3eb9d2b18943297761cbfb690d9ff01e41c360c871a149a9774e74d43ac.png)
...
![](https://raw.githubusercontent.com/AndersonHJB/blog-images/refs/heads/main/docs-images/sha256/5d/5d3a9de7f3986d3b61fad06cbd432acd11e0e960a2432e243b6c6fc255468766.png)
...
```python
/Users/huangjiabao/GitHub/SourceCode/MacBookPro16-Code/PythonCoder/PythonCoderVenv/bin/python /Users/huangjiabao/GitHub/SourceCode/MacBookPro16-Code/PythonCoder/StudentCoder/07cava/多线程/huchisuo.py 
bornforthis.cn
bornforthis.cn
bornforthis.cn
bornforthis.cn
...
start_time = time.time()
for i in range(100):
    t = threading.Thread(target=get_html, args=("bornforthis.cn",))
    t.start()
    t.join()
print(time.time() - start_time)
```
