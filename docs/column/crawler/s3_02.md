---
title: 14-解析无所不能的正则表达式
date: 2022-12-17 17:07:36
author: AI悦创
isOriginal: true
...
操作文件：[regex.ipynb](https://github.com/AndersonHJB/blog-images/blob/main/%E6%9D%82%E7%89%A9/regex.ipynb)
...
```python
Hello, my phone number is 010-86432100 and email is bornforthis@bornforthis.cn, and my website is https://bornforthis.cn.
```

这段字符串中包含了一个电话号码和一个电子邮件，接下来就尝试用正则表达式提取出来，如图所示。

![image-20221217171046429](https://raw.githubusercontent.com/AndersonHJB/blog-images/refs/heads/main/docs-images/sha256/e0/e09efb9b237355a6bf5f99ed702ae6281df44db66eb5b401dceeb282652327e4.png)

在网页右侧选择 “匹配 Email 地址”，就可以看到下方出现了文本中的 E-mail。如果选择 “匹配网址 URL”，就可以看到下方出现了文本中的 URL。是不是非常神奇？

其实，这里使用了正则表达式的匹配功能，也就是用一定规则将特定的文本提取出来。

比方说，电子邮件是有其特定的组成格式的：一段字符串 + @ 符号 + 某个域名。而 URL 的组成格式则是协议类型 + 冒号加双斜线 + 域名和路径。
