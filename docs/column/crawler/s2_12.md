---
title: 12-多路加速，了解多进程基本原理「补充」
date: 2022-11-25 15:59:35
author: AI悦创
isOriginal: true
...
关于信号量那部分生产者和消费者的代码，我的运行结果一直是：Producer append an elementProducer append an element请问这是什么原因呢？

> **回复：** 
>
> 由于是两个进程对信号量进行读写，empty 信号量为 2，full 为 0，消费者每次都要获取 full 的信号量使其减一，生产者要获取 empty 的信号量使其加一，所以。
>
> 运行结果只要执行顺序能满足当前缓冲区大小为 2 即可，消费者和生产者的执行顺序可能不定。
>
> 不过最主要的还是理解信号量的作用和意义。 你的是 windows 吗？如果换到 Linux 或 Mac 应该是好的，具体的解决方案可以见：[https://stackoverflow.com/questions/50379009/python-multiprocessing-semaphore-not-working](https://stackoverflow.com/questions/50379009/python-multiprocessing-semaphore-not-working)，抱歉的确是我当时没有在 WIndows 下单独测试
