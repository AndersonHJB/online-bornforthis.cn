...
方法的返回类型在。 在上面的例子中，返回类型是 int，表示该方法返回一个整数值。 当一个方法返回一个值时，它必须包含一个 return 语句。 在赋值语句中经常使用返回值的方法。
...
2. 第一行之后的每行应包含奇数个星号（1,3,5 等），直到达到行数。「2n-1」
...
最后的 `Console.WriteLine()`;  语句将光标移动到下一行。
...
3. 【填空题】填空，创建一个计算两个参数的乘积的方法，并且给予参数 6 和 8 两个默认值

```cs
static void Test(int x ___ 6, int y= ___) {
...
4. 【填空题】如果一个方法没有返回值,你应该定义方法的返回类型为？
