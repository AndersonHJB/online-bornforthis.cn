...
2. **方法二：使用 `iter()` 函数**
    ...
    **本质**：利用的逻辑与 Python 的强制转换逻辑一致，借助 `iter()` 函数对目标数据类型（变量）进行强制转换，如果目标变量可以被强制转换成功，则代表目标变量为可迭代对象。为什么这么说呢？——比如使用 `int()` 函数可以把字符串转换成数字，但如果使用 int 函数转换字符串出现报错，则意味着：字符串并不是纯数字字符串。——补充日期：2026 年 2 月 9 日 13:23:48 伯克利 GYZ

    我们代码中直接使用 `iter()` 检测列表（`[1, 2, 3]`），是否为可迭代对象。现在是明确检测列表，看起来没有必要。但是我们要了解此方法探究出来的特点，便于后期用于未知数据类型的判断。（什么是未知类型：在代码量较大的程序中，我们有时会需要判断。例如：某程序 1000 行代码执行后，得到的数据类型是什么？是否可迭代？——得到数据类型是什么，可以借助 type，是否可迭代，我们有时无法直观的借助 print 进行输出查看、判断。）

    ```python
    print(type(type([1, 2, 3, 4]))) # 学员提问：type 是什么类型，是 type 类型。所以，我们要知道我们前面提到的 Python 数据类型，都是 Python 最基础的数据类型。
    
    # 数字转换成字符串的所有方法
    num = 10
    
    int_to_str_1 = str(num)
    int_to_str_2 = '{}'.format(num)
    int_to_str_3 = f'{num}'
    int_to_str_4 = '%d' % num
